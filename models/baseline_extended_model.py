...
class ExtendedModel(nn.Module): # give the model a proper name
    """
    Model derived from the ModelIAS with the addition of new classification layers and bi-directional LSTM
    """

    def __init__(self, hid_size, out_slot, out_int, emb_size, vocab_len, n_layer=3, pad_index=0):
        ...
        self.slot_out = SlotsClassifier(hid_size*2, out_slot, dropout_rate=0.1)
        self.intent_out = IntentClassifier(hid_size, out_int, dropout_rate=0.1)
        self.dropout = nn.Dropout(0.1)
        print("Extended baseline model selected")
